...
/**
 * Classifies a stored `device_label` (produced by `@mio/realtime-sdk`'s
 * `guessDeviceLabel()` at registration time — see `registerPortalPushAction`)
 * into "mobile"/"desktop"/"other" for the device management page's icon
 * and filter. Parses the label rather than needing its own backend field,
 * so it also works for rows registered before this classification
 * existed. `null` (no label at all — an older row, or a caller that never
 * sent one) falls into "other", the same as a label naming an OS this
 * can't place either way.
 */
