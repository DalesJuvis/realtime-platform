//! # GetKeysController
//!
//! **Action:** HTTP entry point for `GET /api/v1/portal/keys`.
//! **Input:** Validated `PortalSession`.
//! **Output:** `200 OK` with `KeyPairDto`, or a typed error.
//! **Side effects:** Delegates to `GetMyKeysUseCase`.
...
use crate::modules::portal::usecases::GetMyKeysUseCase;
...
pub async fn handle(State(ctx): State<PortalContext>, Extension(session): Extension<PortalSession>) -> impl IntoResponse {
    match GetMyKeysUseCase::execute(&ctx, session.tenant_id).await {
