...
//!
//! **Action:** HTTP entry point for `GET /api/v1/portal/templates`.
//! **Input:** Validated `PortalSession`.
//! **Output:** `200 OK` with `Vec<TemplateResponseDto>`, or a typed error.
//! **Side effects:** Delegates to `ListTemplatesUseCase`.
...
use crate::modules::portal::usecases::ListTemplatesUseCase;
...
pub async fn handle(State(ctx): State<PortalContext>, Extension(session): Extension<PortalSession>) -> impl IntoResponse {
    match ListTemplatesUseCase::execute(&ctx, session.tenant_id).await {
