...
//!
//! **Action:** HTTP entry point for `POST /api/v1/portal/keys/rotate`.
//! **Input:** Validated `PortalSession`.
//! **Output:** `200 OK` with the new `KeyPairDto`, or a typed error.
//! **Side effects:** Delegates to `RotateMySecretUseCase`.
...
use crate::modules::portal::usecases::RotateMySecretUseCase;
...
pub async fn handle(State(ctx): State<PortalContext>, Extension(session): Extension<PortalSession>) -> impl IntoResponse {
    match RotateMySecretUseCase::execute(&ctx, session.tenant_id).await {
